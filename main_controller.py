...
async def get_product(product_id: int):
    ...
    if not game:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=StatusMessage(content="No entry was found matching your query.").dict())
    return game
...
async def get_all_products_price_range(min_price: float, max_price: float):
    ...
    if not results:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=StatusMessage(content="No products were found matching your min/max price criteria!").dict())
    return results
...
async def search_products(product_price: float, product_type: Optional[str] = None):
    ...
    if not results:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=StatusMessage(content="No products were found from the price or type given!").dict())
    return results
...
async def update_products(products: ProductsRequest):
    ...
    if products.Name == "" or products.Type == "":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=StatusMessage(content="The Request Object was not sent properly!").dict())
